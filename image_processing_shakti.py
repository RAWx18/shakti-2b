...
available_anchor_strategy = ['docshakti', 'random', 'highest', 'last', 'llava']
...
def anchor_rank(anchors, anchors_areas, input_image_size, eps=1e-5):
    ...
    # y2
    boxes3[:,3] = input_image_size[0]/input_image_size[1]*anchors[:,2] # Used to calculate resolution-independent IoU
    ...
    # Prioritize matching similar shapes, then match similar resolutions.
    index = torch.argmax(shape_iou*100+iou,dim=0)
    return index

def select_best_resolution(anchors, anchors_areas, input_image_size): # TO-DO For a futher check
    """
...
class AnchorResize(torch.nn.Module):
    ...
    def __init__(self, image_size, anchors, interpolation=InterpolationMode.BILINEAR, antialias=None, anchor_strategy='docshakti'):
        super().__init__()
    ...
    def forward(self, img, skip_resize=False):
        ...
        """
        if self.anchor_strategy == 'docshakti':
            ...
            # Select the largest area, and on this basis, choose the most square-shaped.
...
class CutMixin:
    def __init__(self, cut_cfg={"anchors": "grid_squ_6", "anchor_strategy": "docshakti", "cut_prompt": "v3", "add_global": True, "cut_prob": 1.0}) -> None:
        if cut_cfg is None:
        ...
        anchor_strategy = cut_cfg.get('anchor_strategy','docshakti')
        ...
        # Remove the scaling from the image processor, and keep only the transformations that follow.
...
class shaktiBatchFeature(BatchFeature):
    r"""
        ...
        def converter(value):
            try:
                if not is_tensor(value):
                    ...
                    return tensor
            except:  # noqa E722 (NO Quality Assurance, do not use bare 'except')
    ...
    def to(self, *args, **kwargs) -> "shaktiBatchFeature":
...
class shaktiImageProcessor(BaseImageProcessor, CutMixin):
    ...
    def preprocess(
        ...
        return shaktiBatchFeature(data={'pixel_values': image_data, 'cut_shape':cut_shape, 'cut_shape_indices':cut_shape_indices})
...
AutoImageProcessor.register("shaktiImageProcessor", shaktiImageProcessor)
