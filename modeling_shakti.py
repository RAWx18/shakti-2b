...
from transformers import Qwen2PreTrainedModel, TextIteratorStreamer
from processing_shakti import shaktiProcessor
from image_processing_shakti import shaktiImageProcessor
from configuration_shakti import shaktiConfig
# from .modeling_navit_siglip import SiglipVisionTransformer
from transformers.models.siglip.modeling_siglip import SiglipVisionTransformer
from modeling_hyper_chakravyuha import HyperChakravyuhaForCausalLM
from torch import nn


class shaktiPreTrainedModel(Qwen2PreTrainedModel):
    config_class = shaktiConfig


class shaktiModel(shaktiPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.language_model = HyperChakravyuhaForCausalLM(config) # Trainable
        self.vision_model = self.init_vision_module() # Trainable
        self.vision_dim = self.vision_model.embed_dim
        self.embed_dim = self.language_model.config.hidden_size
        self.vision2text_model = nn.Linear(self.vision_dim, self.embed_dim) # Trainable
        self.processor = None
    ...
    def init_vision_module(self):
        ...
        self.config.drishti_config._attn_implementation = self.config.drishti_config._attn_implementation
        model = SiglipVisionTransformer(self.config.drishti_config)
    ...
    def init_processor(self, tokenizer):
        ip = shaktiImageProcessor(image_size=384)
        self.processor = shaktiProcessor(image_processor=ip, tokenizer=tokenizer)
